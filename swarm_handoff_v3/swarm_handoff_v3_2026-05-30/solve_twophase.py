...
Phase 1: Assign hub students (>=3 together partners) first
Phase 2: Fix hub assignments and solve remaining students
"""
...
def build_model(data, student_subset, fixed_assignments=None):
    """Build CP-SAT model for a subset of students."""
    ...
    # Also create variables for students with fixed assignments (for capacity counting)
    for sid, assignments in fixed_assignments.items():
        for cid, sec_id in assignments:
            if (sid, sec_id) not in x:
                x[sid, sec_id] = model.NewBoolVar(f'x_{sid}_{sec_id}')
                model.Add(x[sid, sec_id] == 1)  # Fix it
    ...
    for pair in data['separate_pairs']:
        s1, s2 = pair
        if s1 not in student_subset and s2 not in student_subset:
            continue
...
def solve_phase(data, student_subset, fixed_assignments=None, time_limit=120):
    """Solve for a subset of students."""
    model, x = build_model(data, student_subset, fixed_assignments)
    ...
    for (sid, sec_id), var in x.items():
        if sid in student_subset and solver.Value(var):
            cid = data['sections'][sec_id]['course_id']
            assignments[sid].append((cid, sec_id))

    return assignments


def solve_twophase(data):
    """Two-phase solving: hubs first, then rest."""
    print("\n=== PHASE 1: Solving for hub students ===")
    hub_students = data['hub_students']

    # Include all together-connected students in phase 1
    phase1_students = set(hub_students)
    for hub in hub_students:
        phase1_students.update(data['together_graph'][hub])

    print(f"Phase 1 students: {len(phase1_students)} (hubs + their partners)")

    hub_assignments = solve_phase(data, phase1_students, time_limit=180)

    assigned_count = sum(len(a) for a in hub_assignments.values())
    print(f"Phase 1 assigned: {assigned_count} courses to {len(hub_assignments)} students")

    print("\n=== PHASE 2: Solving for remaining students ===")
    remaining_students = set(data['students'].keys()) - phase1_students
    print(f"Phase 2 students: {len(remaining_students)}")

    # Convert hub assignments to fixed format
    fixed = {sid: assignments for sid, assignments in hub_assignments.items()}

    remaining_assignments = solve_phase(data, remaining_students, fixed, time_limit=120)
    ...
    for sid, assigns in remaining_assignments.items():
        for cid, sec_id in assigns:
            all_assignments.append((sid, sec_id))
...
def main():
    print("=== Two-Phase OR-Tools Solver ===\n")
